...
<!-- prettier-ignore -->
> [!NOTE]
> Works in Node 22+ (ESM), Bun 1.3+, and any browser bundler (Vite, webpack, Rollup). A single browser-safe ESM bundle is published — resolution is automatic via the `exports` map.
...
Install dependencies and run the repository-wide quality gate from the monorepo root:
...
For focused work on this package, run Turbo from the monorepo root:
...
Releases are tag-driven. Update this package's version, commit the release, and push a matching
`vX.Y.Z` tag. GitHub Actions validates the complete workspace, checks the package archive and
consumer smoke tests, then publishes this directory through npm trusted publishing with provenance.

The npm trusted publisher must target `stevekinney/octavian` and the workflow filename
`release.yaml`. The publish job is tokenless; do not configure `NPM_TOKEN` or `NODE_AUTH_TOKEN` for
it.
